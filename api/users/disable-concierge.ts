...
async function requireAdminOrSelf(req: VercelRequest, targetFirebaseUid: string) {
  await ensureFirebaseAdmin();
  const token = getBearerToken(req);
  if (!token) throw new Error('missing_auth');
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await requireAdminOrSelf(req, targetUid);
    const supabase = getSupabaseServiceClient();

    // Garantir que é concierge
    ...
    // Só encerrar após a petição ser concluída (approved/completed)
    const { data: donePetition, error: donePetitionError } = await supabase
      .from('petitions')
      .select('id, status')
      .eq('client_id', targetUid)
    ...
    if (!donePetition) {
      return res.status(409).json({ error: 'Concierge só pode ser encerrado após a petição ser concluída.' });
    }

    // 1) Desabilitar login no Firebase + revogar tokens
    await ensureFirebaseAdmin();
