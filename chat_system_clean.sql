-- Script SQL Simplificado para Sistema de Chat
-- Remove tabelas existentes e cria do zero para evitar conflitos
...
DROP FUNCTION IF EXISTS create_conversation(TEXT, VARCHAR(20), UUID, UUID[], VARCHAR(20)[]);
DROP FUNCTION IF EXISTS send_message(UUID, UUID, TEXT, VARCHAR(20), TEXT, VARCHAR(255), INTEGER, UUID);
DROP FUNCTION IF EXISTS mark_message_as_read(UUID, UUID);
DROP FUNCTION IF EXISTS get_user_conversations(UUID);

-- 3. Criar tabela conversations
CREATE TABLE conversations (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    title VARCHAR(255) NOT NULL,
    type VARCHAR(20) NOT NULL DEFAULT 'general' CHECK (type IN ('support', 'petition', 'general')),
...
-- 4. Criar tabela conversation_participants
...
-- 6. Criar tabela message_read_status
...
-- 8. Função para criar uma nova conversa
...
-- 10. Função para marcar mensagem como lida
...
-- 11. Função para obter conversas do usuário
