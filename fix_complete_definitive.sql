...
-- Atualizar sent_at para mensagens existentes que não têm essa coluna
UPDATE messages 
SET sent_at = COALESCE(sent_at, created_at, NOW()) 
WHERE sent_at IS NULL;

-- 2. REMOVER TODAS as funções conflitantes (método mais agressivo)
DO $$
DECLARE
    func_record RECORD;
BEGIN
    ...
    LOOP
        ...
        RAISE NOTICE 'Removida função send_message: %', func_record.args;
    END LOOP;
    ...
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.proname || '(' || func_record.args || ') CASCADE';
        RAISE NOTICE 'Removida função mark_message_as_read: %', func_record.args;
    END LOOP;
    ...
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.proname || '(' || func_record.args || ') CASCADE';
        RAISE NOTICE 'Removida função get_conversation_messages: %', func_record.args;
    END LOOP;
END $$;
...
SELECT 'Correção completa realizada!' as status;
