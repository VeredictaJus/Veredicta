-- Fix para função send_message_v2
-- Verificar se a função existe e corrigir se necessário
...
-- 2. Recriar a função send_message_v2 com melhor tratamento de erros
CREATE OR REPLACE FUNCTION send_message_v2(
    p_conversation_id UUID,
    p_sender_id TEXT,  -- Mudança: usar TEXT para compatibilidade com Firebase UID
    p_content TEXT,
...
BEGIN
    ...
    -- Log para debug
    RAISE NOTICE 'Mensagem inserida com sucesso. ID: %', new_message_id;
...
BEGIN
    ...
    IF test_conversation_id IS NOT NULL THEN
        ...
        SELECT send_message_v2(
            ...
            'test-user-id',
            'Teste da função send_message_v2',
            'text',
            NULL,
            NULL,
            NULL,
...
SELECT 'Função send_message_v2 criada/atualizada com sucesso!' as status;
