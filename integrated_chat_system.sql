...
-- Permite comunicação entre clientes, redatores e admin/suporte

-- Criar tabela de conversas
CREATE TABLE IF NOT EXISTS conversations (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    title VARCHAR(255) NOT NULL,
    type VARCHAR(20) NOT NULL CHECK (type IN ('support', 'petition', 'general')),
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE conversations ADD COLUMN type VARCHAR(20) NOT NULL DEFAULT 'general' CHECK (type IN ('support', 'petition', 'general'));
    END IF;
