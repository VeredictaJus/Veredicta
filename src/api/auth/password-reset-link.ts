...
async function loadFirebaseCredentials(): Promise<FirebaseAdminCredentials> {
  ...
  if (existsSync(LOCAL_JSON_PATH)) {
    try {
      ...
      console.warn('[password-reset-link] Não foi possível ler firebaseAdmin.local.json:', error)
    }
  }
...
async function ensureFirebaseAdmin() {
  ...
  initializing = (async () => {
    ...
    console.log('[password-reset-link] Inicializando Firebase Admin', {
      projectId,
      clientEmail,
      keyLength: normalizedKey.length,
      base64Length: base64Body.length,
      base64Mod4: base64Body.length % 4,
...
export const POST: Handler = async (req, res) => {
  ...
  try {
    ...
    console.log(`[password-reset-link][${requestId}] recebendo request`)
    ...
    console.log(`[password-reset-link][${requestId}] gerando link para ${email}`)
    ...
    console.log(`[password-reset-link][${requestId}] link gerado em ${Date.now() - startedAt}ms`)

    return res.status(200).json({ resetLink: link })
  } catch (error: any) {
    console.error(`[password-reset-link][${requestId}] erro:`, error)

    const message = error?.message || 'Não foi possível gerar o link de reset'
