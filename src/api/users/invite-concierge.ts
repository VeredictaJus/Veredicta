...
import { conciergeInviteEmailTemplate } from '@/services/emailTemplates';
...
function getSupabaseServiceClient() {
  ...
  const supabaseServiceKey =
    process.env.VITE_SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_SERVICE_KEY;
...
export const POST: Handler = async (req, res) => {
  try {
    ...
    // 1) Firebase: buscar ou criar usuário pelo email (sem precisar de UID manual)
    ...
    // 4) Gerar link de definir senha (Firebase) e trocar por link "bonito" da app
    ...
    // 5) Enviar email bonito via Resend (domínio já verificado no projeto)
    ...
    const userName = fullName || companyName || email.split('@')[0];
