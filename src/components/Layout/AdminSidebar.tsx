...
function linkClass(isActive: boolean) {
  ...
  const on = 'bg-amber-500/12 text-amber-800 border-amber-300/50 shadow-sm dark:bg-white/[0.06] dark:text-foreground dark:border-[rgba(255,255,255,0.08)]';
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  />
                  <Icon className={cn('w-4 h-4 transition-colors', isActive ? 'text-amber-600 dark:text-foreground' : 'text-muted-foreground group-hover:text-foreground')} />
                  <span className={cn('flex-1 truncate', isActive ? 'font-semibold' : 'font-medium')}>{label}</span>
                  {isActive && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-amber-500 shadow-sm shadow-amber-400/40 dark:bg-white/80 dark:shadow-white/30" />}
                </>
              )}
            </NavLink>
          ))}
