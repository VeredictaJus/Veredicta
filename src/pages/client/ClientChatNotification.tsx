...
interface ClientChatNotificationProps {
  className?: string;
}

export default function ClientChatNotification({ className }: ClientChatNotificationProps) {
  ...
  useEffect(() => {
    ...
    const previousCount = conversations.reduce((total, c) => total + c.unread_count, 0);
  ...
  }, [unreadCount, conversations, notificationsEnabled, playNotificationSound]);
  ...
  // Se não há mensagens não lidas, não renderizar nada
