...
serve(async (req) => {
  ...
  try {
    ...
    // 4. Função helper para gerar template base (igual ao original)
    const getBaseTemplate = (content: string, COLORS: any): string => {
      return `
        <!DOCTYPE html>
        <html lang="pt-BR">
          <head>
    ...
    // 5. Função helper para gerar template de email (mesmo template do frontend)
